...
import RoomNavigatorModal, { RoomNavigatorModalManager } from "./RoomNavigatorModal"
  ...
  return (
    <div className={classnames(styles["room-link"], { [styles.current]: isCurrent })}>
      ...
      <Link to={path} onClick={(event) => event.stopPropagation()}>
        ...
        {!props.room.read && <span className={styles.unread} />}
      </Link>
    </div>
  )
}
        ...
        {rooms !== null && (
          <>
            ...
            <NavigationRoomSection
              ...
              rooms={rooms.filter(
                (room) => !room.starred && !room.private && room.open && room.location !== null
              )}
            ...
            <NavigationRoomSection
              title="Private"
              rooms={rooms.filter((room) => !room.starred && room.private)}
              showCloseButton={true}
            />
          </>
        )}
